  ...
  for user_arn in "${USER_ARNS[@]}" ; do
    for data_source_id in "${DATASOURCE_IDS[@]}" ; do
      aws quicksight update-data-source-permissions \
                ...
                --revoke-permissions Principal=${user_arn},Actions=quicksight:DescribeDataSource,quicksight:DescribeDataSourcePermissions,quicksight:PassDataSource,quicksight:UpdateDataSource,quicksight:DeleteDataSource,quicksight:UpdateDataSourcePermissions | jq .
    done
  done
else
  for user_arn in "${USER_ARNS[@]}" ; do
